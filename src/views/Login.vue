...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-900 via-slate-900 to-indigo-900 relative overflow-hidden">
    <div class="absolute -top-20 -left-20 w-96 h-96 bg-purple-600 rounded-full blur-3xl opacity-30 animate-pulse"></div>
    <div class="absolute -bottom-20 -right-20 w-96 h-96 bg-indigo-600 rounded-full blur-3xl opacity-30 animate-pulse"></div>
    ...
    <form @submit.prevent="submit" class="relative z-10 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-8 w-full max-w-sm flex flex-col gap-4 shadow-2xl">
      ...
      <button class="bg-gradient-to-r from-purple-600 to-indigo-600 hover:scale-[1.02] active:scale-95 text-white p-3 rounded-lg font-semibold transition">
        Kirish
      </button>

      <router-link to="/register" class="text-white/60 text-sm text-center hover:text-white transition">
        Akkountingiz yo'qmi? Ro'yxatdan o'tish
      </router-link>
    </form>
  </div>
</template>