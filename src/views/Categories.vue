...
import { listCategories, upsertCategory, deleteCategory } from '../services/category'
...
const canUpsert = auth.hasPermission('upsert category')
const canDelete = auth.hasPermission('delete category')
...
async function save() {
  await upsertCategory(form.value)
  showForm.value = false
  await load()
}

async function remove(item) {
  if (!confirm(`"${item.name}" kategoriyasini o'chirishni tasdiqlaysizmi?`)) return
  ...
  <div class="min-h-screen bg-slate-950 text-white p-6">
    <div class="max-w-3xl mx-auto">
      <div class="flex items-center justify-between mb-8">
        <h1 class="text-2xl font-bold text-white">Kategoriyalar</h1>
        <button
            v-if="canUpsert"
            @click="openCreate"
      ...
      <div v-if="showForm" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-slate-800 border border-white/10 rounded-xl p-6 w-full max-w-md">
          <h2 class="text-lg font-semibold mb-4">{{ editItem ? 'Tahrirlash' : 'Yangi kategoriya' }}</h2>
      ...
      <div v-else class="space-y-2">
        ...
        <p v-if="!categories.length" class="text-center py-12 text-white/30">Kategoriyalar mavjud emas</p>
      </div>
    </div>
  </div>
</template>
