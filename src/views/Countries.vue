...
import { listCountries, upsertCountry, deleteCountry } from '../services/country'
...
const canUpsert = auth.hasPermission('upsert country')
const canDelete = auth.hasPermission('delete country')
...
function openEdit(item) {
  editItem.value = item
  form.value = { id: item.id, name: item.name, code: item.code }
  showForm.value = true
}

async function save() {
  await upsertCountry(form.value)
  showForm.value = false
  await load()
}

async function remove(item) {
  if (!confirm(`"${item.name}" mamlakatni o'chirishni tasdiqlaysizmi?`)) return
  ...
  <div class="min-h-screen bg-slate-950 text-white p-6">
    <div class="max-w-3xl mx-auto">
      <div class="flex items-center justify-between mb-8">
        <h1 class="text-2xl font-bold text-white">Mamlakatlar</h1>
        <button
            v-if="canUpsert"
            @click="openCreate"
      ...
      <div v-if="showForm" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-slate-800 border border-white/10 rounded-xl p-6 w-full max-w-md">
          <h2 class="text-lg font-semibold mb-4">{{ editItem ? 'Tahrirlash' : 'Yangi mamlakat' }}</h2>
          <form @submit.prevent="save" class="space-y-4">
            ...
            <div>
              <label class="block text-sm text-white/70 mb-1">Kod (masalan: UZ, US, GB)</label>
              <input
                  v-model="form.code"
                  type="text"
                  class="w-full bg-slate-700 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-400 uppercase"
                  placeholder="UZ"
                  maxlength="5"
              />
            </div>
      ...
      <div v-else class="space-y-2">
        ...
        >
          <div class="flex items-center gap-3">
            <span v-if="item.code" class="text-xs px-2 py-0.5 rounded bg-purple-500/20 text-purple-300 font-mono">{{ item.code }}</span>
        ...
        <p v-if="!countries.length" class="text-center py-12 text-white/30">Mamlakatlar mavjud emas</p>
      </div>
    </div>
  </div>
</template>
