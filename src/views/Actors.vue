...
import { listActors, upsertActor, deleteActor } from '../services/actor'
...
const form = ref({ id: null, name: '', about: '', imageUrl: '' })

const canUpsert = auth.hasPermission('upsert actor')
const canDelete = auth.hasPermission('delete actor')
...
function openCreate() {
  editItem.value = null
  form.value = { id: null, name: '', about: '', imageUrl: '' }
  showForm.value = true
}

function openEdit(item) {
  editItem.value = item
  form.value = { id: item.id, name: item.name, about: item.about, imageUrl: item.imageUrl }
  showForm.value = true
}

async function save() {
  await upsertActor(form.value)
  showForm.value = false
  await load()
}

async function remove(item) {
  if (!confirm(`"${item.name}" aktorni o'chirishni tasdiqlaysizmi?`)) return
  ...
  <div class="min-h-screen bg-slate-950 text-white p-6">
    <div class="max-w-3xl mx-auto">
      <div class="flex items-center justify-between mb-8">
        <h1 class="text-2xl font-bold text-white">Aktorlar</h1>
        <button
            v-if="canUpsert"
            @click="openCreate"
      ...
      <div v-if="showForm" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-slate-800 border border-white/10 rounded-xl p-6 w-full max-w-md">
          <h2 class="text-lg font-semibold mb-4">{{ editItem ? 'Tahrirlash' : 'Yangi aktyor' }}</h2>
          <form @submit.prevent="save" class="space-y-4">
            <div>
              <label class="block text-sm text-white/70 mb-1">Ismi</label>
              <input
                  v-model="form.name"
                  type="text"
                  required
            ...
            <div>
              <label class="block text-sm text-white/70 mb-1">Biografiya</label>
              <textarea
                  v-model="form.about"
                  rows="3"
                  class="w-full bg-slate-700 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-400 resize-none"
                  placeholder="Aktyor haqida"
              />
            </div>
            <div>
              <label class="block text-sm text-white/70 mb-1">Rasm URL</label>
              <input
      ...
      <div v-else class="space-y-2">
        ...
        >
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full overflow-hidden bg-slate-800 border border-white/10 shrink-0">
              <img v-if="item.imageUrl" :src="item.imageUrl" alt="" class="w-full h-full object-cover" />
              <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-full h-full text-white/20 p-2" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" />
              </svg>
            </div>
            <div>
              <p class="text-sm font-medium">{{ item.name }}</p>
              <p v-if="item.about" class="text-xs text-white/40 mt-0.5 truncate max-w-xs">{{ item.about }}</p>
            </div>
          </div>
        ...
        <p v-if="!actors.length" class="text-center py-12 text-white/30">Aktorlar mavjud emas</p>
      </div>
    </div>
  </div>
</template>
