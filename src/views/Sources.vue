...
import { listSources, upsertSource, deleteSource } from '../services/source'
...
const canUpsert = auth.hasPermission('upsert source')
const canDelete = auth.hasPermission('delete source')
...
async function save() {
  await upsertSource(form.value)
  showForm.value = false
  await load()
}

async function remove(item) {
  if (!confirm(`"${item.name}" manbani o'chirishni tasdiqlaysizmi?`)) return
  ...
  <div class="min-h-screen bg-slate-950 text-white p-6">
    <div class="max-w-3xl mx-auto">
      <div class="flex items-center justify-between mb-8">
        <h1 class="text-2xl font-bold text-white">Manbalar</h1>
        <button
            v-if="canUpsert"
            @click="openCreate"
      ...
      <div v-if="showForm" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-slate-800 border border-white/10 rounded-xl p-6 w-full max-w-md">
          <h2 class="text-lg font-semibold mb-4">{{ editItem ? 'Tahrirlash' : 'Yangi manba' }}</h2>
          <form @submit.prevent="save" class="space-y-4">
            <div>
              ...
              <input
                  ...
                  placeholder="Manba nomi (masalan: VK, Telegram)"
              />
            </div>
            <div>
      ...
      <div v-else class="space-y-2">
        ...
        <p v-if="!sources.length" class="text-center py-12 text-white/30">Manbalar mavjud emas</p>
      </div>
    </div>
  </div>
</template>
